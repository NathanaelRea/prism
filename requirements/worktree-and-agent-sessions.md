...
- **Behavior**: The main panel shows the selected worktree's current OpenCode
  workflow status, latest user message, and up to five latest assistant messages.
  Empty content retains stable layout.
...
- **Behavior**: Users can enter a selected Plan step's corresponding OpenCode
  session in tmux.
