...
The compiled, content-addressed run snapshot pins authored prompts, selected
harness/model/variant values, dependencies, context selections, and external
Trigger executable bytes.

A Step is an Agent invocation with an optional **Trigger** lifecycle adapter. A
Trigger observes whether its Step should `Run`, is `Satisfied`, should `Wait`,
...
completed. Every Agent start uses a fresh native session and consumes one
persisted run budget unit.
...
- Agent prompts are sent as authored. Only explicitly selected predecessor final
  messages are appended as labeled plain-text sections; no evidence JSON or
  required structured result is added.
...
- Persisting prepared state before Agent start permits restart at the Agent
  phase. An uncertain non-repeatable external hook becomes
  `recovery_required` instead of being repeated blindly.
