...
- `W` opens one flat `fzf` picker of hot-discovered prompt Workflows for the selected Worktree Session. `Enter` opens the typed input form, then runs the selected Workflow; `Ctrl-E` edits its source. The picker shows source scope and path. In the form, `Tab`/arrow keys move, text and numbers edit inline, Space/left/right toggles booleans, enum fields open a dropdown, and file fields open `fzf`.
- `Space w w` opens the same Workflow picker. `Space w a` opens the AI one-off Workflow creator for the selected Worktree Session. Describe the work, then preview, run, edit, or regenerate the generated TOML. Drafts live in Prism state and are cleaned up with the Worktree Session. Parallel Steps are experimental: they run concurrently in the same worktree and may edit overlapping files.
