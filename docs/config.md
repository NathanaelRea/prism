...
Worktrunk's personal worktree path policy belongs in its user configuration. Before Prism first tracks a repository, it asks Worktrunk for the user-config path and project identifier, creates the user config through `wt config create` when needed, and atomically adds an empty `[projects."<identifier>"]` entry while preserving existing content. Concurrent Prism registrations serialize on Worktrunk's user-config lock; a setup failure leaves the repository untracked. Prism does not create or modify the repository's `.config/wt.toml`. Choose Worktrunk configuration under `Space c` to open the user file; the dialog makes explicit that changes affect both Prism and standalone `wt` commands. Use Worktrunk's top-level `worktree-path` for a global policy or fill in the registered project table for a personal repository override.
