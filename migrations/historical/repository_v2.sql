...
CREATE TABLE task_metadata (
        ...
        , classification text not null default 'work', visibility integer not null default 0);
...
CREATE TABLE pr_cache (
        ...
        , requested_reviewers text not null default '', merge_state_status text not null default '', observation_error text, author text not null default '', queue_state text not null default '', native_state_evidence text not null default '{}', provider text, canonical_host text, project_path text, native_cr_id text, display_number integer, source_provider text, source_canonical_host text, source_project_path text, target_provider text, target_canonical_host text, target_project_path text, identity_complete integer not null default 0);
CREATE TABLE pr_details_cache (
          ...
          refreshed_unix_ms integer not null
        , ci_failures text not null default '[]', check_contexts text not null default '[]', pr_number integer, head_sha text, observation_error text, provider text, canonical_host text, project_path text, native_cr_id text, display_number integer, source_provider text, source_canonical_host text, source_project_path text, target_provider text, target_canonical_host text, target_project_path text, identity_complete integer not null default 0);
CREATE TABLE plan_run (
        ...
        , harness_id text not null default 'opencode', adapter_id text not null default 'opencode', worktree_session_id text);
CREATE TABLE plan_step_run (
          ...
          summary text,
          error text, opencode_state text, agent_variant text, execution_state text, execution_process_id integer, execution_process_start_time_ticks integer, session_endpoint text, session_id text, session_adapter_id text,
...
CREATE TABLE auto_run (
          ...
          created_unix_ms integer not null,
          updated_unix_ms integer not null,
          archived_unix_ms integer, implementation_source text not null default 'prompt', plan_path text, plan_run_mode text not null default 'sequential', stabilization_status text, stabilization_blocker text, stabilization_next_work text, pending_push_json text, harness_id text not null default 'opencode', adapter_id text not null default 'opencode', worktree_incarnation text, change_request_identity_json text, worktree_session_id text,
...
CREATE TABLE auto_step_run (
          ...
          head_sha text,
          summary text,
          error text, plan_run_id text, work_guard_json text, blocker text, execution_process_id integer, execution_process_start_time_ticks integer, session_endpoint text, session_id text, session_adapter_id text, execution_state text,
...
CREATE TABLE repo_policy_cache (
          ...
          refreshed_unix_ms integer not null,
          error text
        , provider text, canonical_host text, project_path text, target_branch text, identity_complete integer not null default 0);
...
CREATE TABLE opencode_runtime (
                   ...
                   updated_unix_ms integer not null,
                   server_start_time_ticks integer, worktree_session_id text,
...
CREATE TABLE workflow_execution (
          ...
          created_unix_ms integer not null,
          updated_unix_ms integer not null, execution_version integer not null default 1, not_before_unix_ms integer, wake_reason text, workflow_revision integer not null default 0,
