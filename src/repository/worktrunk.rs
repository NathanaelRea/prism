...
mod tests {
    ...
    async fn real_worktrunk_create_observe_remove_smoke() {
        ...
        create_user_config(&repo, &config).await.unwrap();
        assert!(discover_user_config(&repo, &config).await.unwrap().exists);
    ...
    #[tokio::test]
    async fn config_show_rejects_missing_or_empty_user_config_locations() {
        for fixture in [
            r#"{}"#,
            r#"{"user":null}"#,
