...
pub(crate) async fn fetch_change_request_branch(
    ...
    if destination_old_oid != *"0000000000000000000000000000000000000000" {
        if destination_old_oid == summary.head_sha {
            return Ok(());
        }
        return Err(format!(
            "destination branch {branch} already exists at a different commit"
        ));
    }
...
pub(crate) async fn capabilities(path: &Path, config: &Config) -> Result<Capabilities, String> {
    ...
    match adapter {
        Adapter::Forgejo(adapter) => {
            let adapter = (*adapter).clone();
            let repository = remote.repository.id;
            forgejo_call(RemoteOperation::DiscoverRepository, move || {
                adapter
                    .runtime_diagnostics(&repository)
...
async fn prepare_create_change_request(
    path: &Path,
    config: &Config,
    branch: &str,
    target_repository: &RemoteRepositoryId,
...
pub(crate) async fn merge_change_request(
    ...
    .await?;
    execute_guarded_merge(config, path, &request).await
}

pub(crate) async fn prepare_merge_change_request(
    config: &Config,
    path: &Path,
    authorized_identity: &CanonicalChangeRequestIdentity,
    display_number: u64,
...
pub(crate) async fn execute_guarded_merge(
    config: &Config,
    path: &Path,
    request: &GuardedMerge,
) -> Result<MergeMutationResult, String> {
    match execute_guarded_merge_reconciled(config, path, request).await {
...
pub(crate) async fn execute_guarded_merge_reconciled(
    config: &Config,
    path: &Path,
    request: &GuardedMerge,
    ...
    let error = match adapter.merge_change_request(request).await {
        Ok(result) => return GuardedMergeExecution::Applied(Box::new(result)),
        Err(error) => error.to_string(),
    };
    let observed = match adapter.observe_change_request(&request.id).await {
...
"#,
    ...
    #[cfg(unix)]
    #[tokio::test(flavor = "multi_thread")]
    async fn existing_destination_branch_at_another_head_is_rejected() {
        let directory = std::env::temp_dir().join(format!(
            "prism-fetch-existing-mismatch-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap()
...
"#,
        ...
        assert!(
            error.contains("already exists at a different commit"),
            "{error}"
        );
        let commands = std::fs::read_to_string(&log).unwrap();
        assert!(!commands.contains("fetch origin"));
