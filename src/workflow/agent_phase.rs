...
async fn execute_invocation(
    ...
    let mut child = super::child_process::spawn(&mut command).map_err(AgentExecutionError::Io)?;
    let process_id = super::child_process::id(&child);
    let mut process_guard = super::child_process::DropGuard::new(&child);
    if let Some(input) = &invocation.stdin {
        let mut stdin = super::child_process::stdin(&mut child)
            .take()
            .ok_or_else(|| {
    ...
    let status = loop {
        ...
        match tokio::time::timeout(
            ...
            super::child_process::wait(&mut child),
        )
        .await
        {
